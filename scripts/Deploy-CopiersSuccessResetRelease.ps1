...
if($release.ScmBaseUrl -ne $scm -or $release.SubscriptionId -ne $subscription -or $release.BaselineDeploymentId -ne '57dab2ecc77042969afc2c359486c06a' -or $release.Files.Count -ne 6 -or $release.ConfigurationChanges){throw 'Unapproved release manifest.'}
...
$marker='copiers-success-reset-'+$release.SourceCommit.Substring(0,8)+'-'+[Guid]::NewGuid().ToString('N')
