...
if($release.ScmBaseUrl -ne $scm -or $release.SubscriptionId -ne $subscription -or $release.BaselineDeploymentId -ne 'c488553429a6466ea6e05a48b3ae8d5a' -or $release.Files.Count -ne 9 -or $release.ConfigurationChanges -or $release.ConfigurationScope -ne 'None'){throw 'Unapproved release manifest.'}
...
$expected=@('CotizadorInterno.Web.dll','CotizadorInterno.Web.pdb','CotizadorInterno.Web.staticwebassets.endpoints.json')+@($assets|ForEach-Object {$_;"$_.br";"$_.gz"})
...
$marker='conciliacion-contact-'+$release.SourceCommit.Substring(0,8)+'-'+[Guid]::NewGuid().ToString('N')
...
try{
    ...
    Write-Output 'Deploying the frozen Conciliacion contact and import-detail package; configuration is preserved.'
