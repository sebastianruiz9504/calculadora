...
if($release.ScmBaseUrl -ne $scm -or $release.SubscriptionId -ne $subscription -or $release.BaselineDeploymentId -ne 'f8548c51948e45d6a59b37988d0136fa' -or $release.Files.Count -ne 12 -or $release.ConfigurationChanges){throw 'Unapproved release manifest.'}
...
$expected=@('CotizadorInterno.Web.dll','CotizadorInterno.Web.pdb','CotizadorInterno.Web.staticwebassets.endpoints.json') + @(@('wwwroot/js/copiers.js','wwwroot/js/copiers-mto-v2-calendar.js','wwwroot/js/dashboard.js') | ForEach-Object { $_; "$_.br"; "$_.gz" })
...
try{
    ...
    Write-Output 'Deploying frozen twelve-file package once; configuration is unchanged.'
