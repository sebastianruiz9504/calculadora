param(
    ...
    [string]$SchemaPath = "$PSScriptRoot\pnl-manual-items-dataverse-schema.json",
...
function New-AttributePayload($column) {
    ...
    switch ($column.type) {
        ...
        "Choice" {
            $options = @($schema.PSObject.Properties[$column.optionsRef].Value)
            $base["@odata.type"] = "Microsoft.Dynamics.CRM.PicklistAttributeMetadata"
            $base.OptionSet = @{
                "@odata.type" = "Microsoft.Dynamics.CRM.OptionSetMetadata"
                IsGlobal = $false
                OptionSetType = "Picklist"
                Options = @($options | ForEach-Object { New-OptionMetadata $_.label ([int]$_.value) })
            }
            return $base
        }
        "DateOnly" {
...
function New-PnlManualEntity {
    ...
    $payload = @{
        ...
        Description = New-DvLabel "Registros manuales para Rebates e Ingresos financieros del P&L."
...
Write-Host "Provision de registros P&L manuales finalizada." -ForegroundColor Green
