...
if($release.ScmBaseUrl -ne $scm -or $release.SubscriptionId -ne $subscription -or $release.BaselineDeploymentId -ne '914bc93575654c7882f65b4d228fea46' -or $release.Files.Count -ne 16 -or !$release.ConfigurationChanges -or $release.ConfigurationScope -ne 'CopiersEquipmentOperations-only'){throw 'Unapproved release manifest.'}
...
$expected=@('CotizadorInterno.Web.dll','CotizadorInterno.Web.pdb','CotizadorInterno.Web.staticwebassets.endpoints.json','appsettings.json')+@($assets|ForEach-Object {$_;"$_.br";"$_.gz"})
...
try{
    ...
    Write-Output 'Deploying one frozen equipment-operations package; only the approved feature configuration changes.'
