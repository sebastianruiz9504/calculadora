...
if($release.ScmBaseUrl -ne $scm -or $release.SubscriptionId -ne $subscription -or $release.BaselineDeploymentId -ne 'a9121dd876254332b5ab1ef332254b75' -or $release.Files.Count -ne 7 -or $release.ConfigurationChanges){throw 'Unapproved release manifest.'}
...
$expected=@('CotizadorInterno.Web.staticwebassets.endpoints.json') + @(@('wwwroot/js/dashboard.js','wwwroot/css/dashboard.css') | ForEach-Object { $_; "$_.br"; "$_.gz" })
...
try{
    ...
    Write-Output 'Deploying frozen seven-file static package once; configuration is unchanged.'
